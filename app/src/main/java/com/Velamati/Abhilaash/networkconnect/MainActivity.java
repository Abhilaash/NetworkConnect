...
//        textview = (TextView) findViewById(R.id.textview);
    ...
    private void display()
    {
        ...
        for (int x = 0; x < json.length(); x++) {
            try {
                headers.add(json.getJSONObject(x).getString("notamnumber") + "|" + json.getJSONObject(x).getString("notamtext"));
                values = new ArrayList<String>();
                Iterator<String> y = json.getJSONObject(x).keys();
                while(y.hasNext()) {
                    String str = y.next();
                    if(!headers.contains(json.getJSONObject(x).getString(str)) && !json.getJSONObject(x).getString(str).equals(""))
